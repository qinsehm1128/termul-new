...
The ACP Core owns Conversation lifecycle authority while Terminal Core owns PTYs. Suspend performs an operation-time scoped observation and never terminates PTYs. Delete is a durable, orchestrated Saga backed by the ACP-local lifecycle journal: it persists capture → observe → terminate pending → termination confirmed → purge pending → completed transitions under a deterministic operation ID. ACP restart can resume unfinished operations; `AlreadyGone` is successful cleanup, while unavailable, unknown, ownership-mismatched, cleanup-failed, or still-live terminal state is fail-closed and never permits purge. This is eventual convergence and idempotent retry, not a cross-process 2PC transaction or rollback guarantee for Unix process side effects. Unix integration coverage starts both real Core processes, provisions a conversation-scoped terminal and workspace reference, then verifies ACP deletion removes the Terminal Core PTY before purging the Conversation. HTTP, WS, and Tauri lifecycle transports delegate to this same authority and only remove a requested workspace directory after an explicit `Deleted` outcome. Shared-live `/terminal/ws` uses the TerminalServiceHandle in Core-backed mode and the local manager only for in-process fallback.

When Terminal Core restarts and a declared Conversation terminal is no longer observable, ACP can persist a policy-safe host spawn intent and create a replacement PTY, then atomically replace the SessionWorkspace terminal reference and topology identity. This Conversation-level recovery preserves ownership scope and user workflow, but does not preserve the original shell, PID, PTY master FD, in-memory process state, or complete output continuity. Reliable original-PTY continuity requires a stable PTY owner such as a future broker or multiplexer; that architecture is intentionally deferred and is not implemented by the recovery journal. Unix/macOS recovery is covered here; Windows named-pipe transport remains compile-level plumbing, desktop activation is deferred and logs `ACTIVATION_DEFERRED` for the explicit in-process fallback (not Core runtime acceptance). Packaged dual-role smoke remains outside this development acceptance path. Crash/update behavior is in the matrix above; fd-passing Core handoff is deferred.
